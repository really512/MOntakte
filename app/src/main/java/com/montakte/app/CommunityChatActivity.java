...
public class CommunityChatActivity extends Activity {
    ...
    private void renderMessage(String uid,String body){TextView v=text("👤 Загрузка...\n"+(body==null?"":body),16);messages.addView(v);if(uid==null){v.setText("👤 Пользователь\n"+(body==null?"":body));return;}db.collection("users").document(uid).get().addOnSuccessListener(p->{String n=p.getString("displayName"),phone=p.getString("phone"),avatar=p.getString("avatar");if(n==null||n.trim().isEmpty())n=phone==null?"Пользователь":phone;String icon=(avatar==null||avatar.trim().isEmpty())?"👤":avatar;v.setText(icon+" "+n+"\n"+(body==null?"":body));}).addOnFailureListener(e->v.setText("👤 Пользователь\n"+(body==null?"":body)));}
