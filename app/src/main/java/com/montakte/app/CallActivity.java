...
public class CallActivity extends Activity {
 ...
 private FirebaseFirestore db; private FirebaseAuth auth; private String peerUid,callId; private boolean videoCall,incoming,closing;
 private PeerConnectionFactory factory; private PeerConnection peer; private AudioSource audioSource; private AudioTrack audioTrack; private VideoSource videoSource; private VideoTrack videoTrack; private CameraVideoCapturer cameraCapturer; private SurfaceTextureHelper surfaceHelper;
 ...
 private void setupCamera(){videoSource=factory.createVideoSource(false);cameraCapturer=createCameraCapturer();if(cameraCapturer==null)throw new IllegalStateException("Камера не найдена");surfaceHelper=SurfaceTextureHelper.create("CallCamera",factory.getNativePeerConnectionFactory()==0?null:null);}
 ...
 private void putSignal(String key,SessionDescription s){Map<String,Object> x=new HashMap<>();x.put("type",s.type.canonicalForm());x.put("sdp",s.description);db.collection("calls").document(callId).update(key,x);if("offer".equals(key))listenForAnswer();}
 private void listenForAnswer(){answerListener=db.collection("calls").document(callId).addSnapshotListener((d,e)->{if(e!=null||d==null||!d.exists()||peer==null)return;Map<String,Object>a=(Map<String,Object>)d.get("answer");if(a!=null&&peer.getRemoteDescription()==null)peer.setRemoteDescription(new EmptySdp(),new SessionDescription(SessionDescription.Type.ANSWER,String.valueOf(a.get("sdp"))));});}
 private boolean caller(){return auth.getCurrentUser().getUid().equals(getCallerUid());}
 private String getCallerUid(){return auth.getCurrentUser().getUid();}
 private void writeCandidate(IceCandidate c){if(callId==null)return;Map<String,Object>x=new HashMap<>();x.put("sdpMid",c.sdpMid);x.put("sdpMLineIndex",c.sdpMLineIndex);x.put("candidate",c.sdp);String col=caller()?"callerCandidates":"calleeCandidates";db.collection("calls").document(callId).collection(col).add(x);}
 private void listenIce(){if(callId==null)return;if(callerIceListener==null)callerIceListener=db.collection("calls").document(callId).collection("callerCandidates").addSnapshotListener((s,e)->{if(e==null&&s!=null)for(DocumentSnapshot d:s.getDocuments())addCandidate(d);});if(calleeIceListener==null)calleeIceListener=db.collection("calls").document(callId).collection("calleeCandidates").addSnapshotListener((s,e)->{if(e==null&&s!=null)for(DocumentSnapshot d:s.getDocuments())addCandidate(d);});}
