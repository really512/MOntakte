...
public class BotChatActivity extends Activity {
    ...
    private void loadMessages(){db.collection("bots").document(botId).collection("messages").orderBy("createdAt",Query.Direction.ASCENDING).limitToLast(100).addSnapshotListener((s,e)->{if(e!=null||s==null)return;messages.removeAllViews();for(DocumentSnapshot d:s.getDocuments()){boolean bot=Boolean.TRUE.equals(d.getBoolean("fromBot"));addBubble(d.getString("text"),bot);}});}
    ...
    private void send(){String s=input.getText().toString().trim();if(s.isEmpty())return;input.setText("");Map<String,Object> m=new HashMap<>();m.put("senderUid",user.getUid());m.put("text",s);m.put("fromBot",false);m.put("createdAt",FieldValue.serverTimestamp());db.collection("bots").document(botId).collection("messages").add(m).addOnSuccessListener(x->{String reply=replyFor(s);Map<String,Object> r=new HashMap<>();r.put("senderUid",botId);r.put("text",reply);r.put("fromBot",true);r.put("createdAt",FieldValue.serverTimestamp());db.collection("bots").document(botId).collection("messages").add(r);}).addOnFailureListener(e->Toast.makeText(this,"Не удалось отправить",Toast.LENGTH_SHORT).show());}
    private String replyFor(String s){String q=s.toLowerCase();if(q.equals("/start")||q.contains("привет")||q.contains("hello"))return "Привет! 🤖 Я бот ВОнтакте. Напиши /help.";if(q.equals("/help")||q.contains("помощь"))return "Команды: /start — приветствие, /help — помощь, /time — текущее время.";if(q.equals("/time"))return "Сейчас: "+new java.text.SimpleDateFormat("HH:mm",java.util.Locale.getDefault()).format(new java.util.Date());if(q.startsWith("/echo "))return s.substring(6);return "Я получил: «"+s+"». 🤖";}
}
